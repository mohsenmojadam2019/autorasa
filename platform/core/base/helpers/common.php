...
if (! function_exists('remain_qouta')) {
    function remain_qouta($user_id): int
    {
        $lastYear = now()->subYear();
        $orders = Order::where('user_id', $user_id)->where('is_finished', '>', 0)->pluck('id');
        $products = \Botble\Ecommerce\Models\Product::where('has_qouta', 1)->pluck('id');
        ...
        return max_qouta()-$totalQty;
    }
}
if (! function_exists('max_qouta')) {
    function max_qouta(): int
    {
        return 4;
    }
}
