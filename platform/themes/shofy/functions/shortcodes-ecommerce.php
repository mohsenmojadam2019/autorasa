...
//
    Shortcode::register('ecommerce-dimensions', __('Ecommerce Dimensions'), __('Ecommerce Dimensions'), function (ShortcodeCompiler $shortcode) {
        $dimensionIds = Shortcode::fields()->getIds('dimension_ids', $shortcode);

        $dimensions = Dimension::query()
            ->wherePublished()
            ->when($dimensionIds, fn ($query) => $query->whereIn('id', $dimensionIds))
            ->get();
        ...
        return Theme::partial('shortcodes.ecommerce-dimensions.index', compact('shortcode', 'dimensions'));
    });

    Shortcode::setAdminConfig('ecommerce-dimensions', function (array $attributes) {
        return ShortcodeForm::createFromArray($attributes)
            ->withLazyLoading()
            ->add(
                'title',
            ...
            ->add(
                ...
                SelectFieldOption::make()
                    ...
                    ->label(__('Choose dimensions'))
                    ->selected(ShortcodeField::parseIds(Arr::get($attributes, 'dimension_ids')))
                    ->helperText(__('Leave empty to show all dimensions'))
                    ->searchable()
                    ->multiple()
            )
            ->add(
