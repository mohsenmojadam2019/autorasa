                    ...
                    <div class="cartmini__widget-item">
                        ...
                        <div class="cartmini__content">
                            ...
                            <h5 class="cartmini__title">
                                <a href="{{ $product->original_product->url }}" style="color: #212121 !important;">{{ $product->original_product->name }}</a>
                            </h5>
                            @include(Theme::getThemeNamespace('views.ecommerce.includes.cart-quantity'),$product)
                            <div class="cartmini__price-wrapper">
                                ...
                                {{--                            ])--}}
                                <span style="display: block; margin-top: 12px; font-size: 14px; color: #666;">
{{--                                  {{ __('price per unit') }}: <strong>{{ number_format($product->price) }}</strong> {{ __(':currency', ['currency' => get_application_currency()->symbol]) }}--}}
                                  {{ __('قیمت هر واحد') }}: <strong>{{ number_format($product->price) }}</strong> {{ __(':currency', ['currency' => get_application_currency()->symbol]) }}
                                </span>
...
@else
    <div class="cartmini__empty text-center">
        <img src="{{ theme_option('ecommerce_empty_cart_image') ? RvMedia::url(theme_option('ecommerce_empty_cart_image')): Theme::asset()->url('images/empty-cart.png') }}"
             alt="{{ __('Empty Cart') }}">
