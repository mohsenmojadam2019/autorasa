...
//dd(Cart::instance('cart'));
    ...
    )
    {
        ...
        foreach ($products as $product) {
            ...
            if ((remain_qouta(\auth()->guard('customer')->id()) < 1) or $quantityOfProduct > remain_qouta(\auth()->guard('customer')->id())) {
                return $this
                    ->httpResponse()
                    ->setError()
                    ->setMessage(
                        __('Remain order quantity of product :product is :quantity! ', [
                            'product' => $product->original_product->name,
                            'quantity' => remain_qouta(\auth()->guard('customer')->id()),
                        ])
                    );
            }
        }
...
//edit
        $this->updateOrderAddressWithBookingData($order);
    ...
    public function getCheckoutSuccess(string $token)
    {
        ...
        $message = "$name عزیز، محصول شما با موفقیت خریداری شد. کد رهگیری شما $code است. اتوراسا";
    ...
    public function services()
    {
        ...
        return Theme::scope('ecommerce.services.services')->render();

    }

    public function serviceCenter()
    {
        // نمایش فرم یا اطلاعات مرتبط با نصب در مرکز خدمات
        return Theme::scope('ecommerce.services.service-center')->render();

    }

    public function atHome()
    {
        // نمایش فرم یا اطلاعات مرتبط با نصب در محل
        return Theme::scope('ecommerce.services.on-spot')->render();
    ...
    public function address()
    {
        ...
        return Theme::scope('ecommerce.services.on-spot.address.address')->render();
    }
    ...
    public function submitAddress(Request $request)
    {
        ...
        $city = DB::table('cities')->where('id', $request->input('city_id'))->value('name');

        // Check if province and city exist
        if (!$province || !$city) {
            return redirect()->back()->with('error', 'استان یا شهر معتبر نیست.');
        }
        ...
        Address::updateOrCreate(
            ...
            [
                ...
                'state' => $province, // Use the province name
                'city' => $city, // Use the city name
        ...
        return redirect()->route('public.checkout.address.show', ['token' => $request->route('token')])
            ->with('success', 'اطلاعات آدرس با موفقیت ثبت شد.');
    }
    ...
    public function showAddress(Request $request)
    {
        ...
        return Theme::scope('ecommerce.services.on-spot.address.address-show')->render();
    ...
    {
        return Theme::scope('ecommerce.services.on-spot.booking.booking-car')->render();
    }

    public function submitBookingCar(Request $request)
    {
//        dd($request);
        if (!EcommerceHelper::isEnabledGuestCheckout() && !auth('customer')->check()) {
            return $this
...
//            Cart::instance('cart')->destroy();
            ...
            return redirect()->route('public.checkout.select-car', ['token' => $request->route('token')])
                ->with('success', 'اطلاعات آدرس با موفقیت ثبت شد.');
        } catch (\Exception $e) {
            dd($e->getMessage());
            DB::rollBack();
    ...
    public function submitBookingAutoservice(Request $request)
    {
//        dd($request);
        if (!EcommerceHelper::isEnabledGuestCheckout() && !auth('customer')->check()) {
            return $this
...
//            Cart::instance('cart')->destroy();
            ...
            DB::commit();
            return response()->json(['message'=>'اطلاعات آدرس با موفقیت ثبت شد.','data'=>[]]);
//            return redirect()->route('public.checkout.select-car', ['token' => $request->route('token')])
//                ->with('success', 'اطلاعات آدرس با موفقیت ثبت شد.');
        } catch (\Exception $e) {
            dd($e->getMessage());
    ...
    public function selectCar()
    {
        ...
        return Theme::scope('ecommerce.services.on-spot.booking.car-select')->render();
    }
