...
//        $products = $productService->getProduct($request, null, null, null, $with);
//
//        $productstemp = new Collection();
//
//        foreach ($products->load('fitmentAttributes') as $product) {
//            $matchedAll = collect($request->fitments)->every(function ($fitment, $key) use ($product) {
//                return $product->fitmentAttributes->first(function ($item) use ($key, $fitment) {
//                    return $item->pivot->attribute_name == $key && Str::contains($item->pivot->value, $fitment);
//                });
//            });
//
//            if ($matchedAll) {
...
//                dd($products->all());
                ...
                $flag = true;
                $productFitAtr = DB::table('ec_product_fitment_attribute')
                    ->where('product_id', $product->id)
                    ->get();
//dd($request->fitments );
                foreach ($request->fitments as $key => $fitment) {
                    $matched = $productFitAtr->first(function ($item) use ($key, $fitment) {
//                        dd($item,$fitment,$key);
                        return $item->attribute_id == $key && $item->option_id == $fitment;
//                        Str::contains($item->value, $fitment);
                    });
...
//dd($productstemp);
            // تبدیل $productstemp به یک Collection از مدل‌های Eloquent و سپس بارگذاری برند
            $productstemp = Product::find($productstemp->pluck('id'));  // تبدیل به مدل‌های Eloquent

            // بارگذاری برند برای محصولات
            $productstemp->loadMissing(['brand']);

            $paginated = new \Illuminate\Pagination\LengthAwarePaginator(
                $productstemp->forPage(1, 15),
                $productstemp->count(),
                15,
                1,
                ['path' => $products->path(), 'perPage' => $products->perPage()]
            );

//            dd($productstemp, $paginated->loadMissing(['brand']), $products->loadMissing(['brand']));
