...
class Product extends BaseModel
{
    ...
    {
        return $this->belongsTo(Dimension::class)->withDefault();
    }
    ...
    public function specificationAttributes(): BelongsToMany
    {
        return $this
            ...
            ->withPivot('value', 'hidden','show_in_detail', 'show_in_card', 'order')
            ->orderBy('ec_product_specification_attribute.order');
    }

    public function fitmentAttributes(): BelongsToMany
    {
        return $this
            ->belongsToMany(FitmentAttribute::class, 'ec_product_fitment_attribute', 'product_id', 'attribute_id')
            ->withPivot( 'attribute_name', 'order')
            ->orderBy('ec_product_fitment_attribute.order');
    }
    public function fitments()
    {
        return $this->hasMany(FitmentProduct::class, 'product_id')->with(['attribute', 'option']);
    }
    ...
    {
        return $this->belongsToMany(Order::class, 'ec_order_product')
            ->withPivot(['qty', 'price', 'tax_amount', 'options'])
            ->withTimestamps();
    }
    ...
    public function remainQouta()
    {
//        dd($this->categories());
return 4;
//        if (($this->maximum_order_quantity<1) or (!\auth()->guard('customer')->check())){
//
//            return $this->maximum_order_quantity>0 ?$this->maximum_order_quantity:$this->quantity;
//        }
//
//        $lastYear = now()->subYear();
//        $orders = Order::where('user_id', \auth()->guard('customer')->id())->where('is_finished', '>', 0)->pluck('id');
//        $products = Product::where('has_qouta', 1)->id()->pluck('id');
//
//        $totalQty = OrderProduct::whereIn('order_id', $orders)
...
//    public function validQouta(): bool
//    {
//
//        if (($this->maximum_order_quantity<1)or(!\auth()->guard('customer')->check()))
//            return true;
...
////            })
////            ->sum('qty');
////        dd($totalQty,$totalQty < $this->maximum_order_quantity);
//
//        return $totalQty < $this->maximum_order_quantity? true:false ;
//    }
