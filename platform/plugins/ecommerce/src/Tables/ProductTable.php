...
class ProductTable extends TableAbstract
{
    public function setup(): void
    {
        $this
            ...
            ->addColumn(Column::make('tire_size')
                ->title('سایز لاستیک')
                ->alignLeft())
            ->addColumn(Column::make('tire_year')->title('سال'))
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('tire_size', function (Product $item) {
                $size = $item->specificationAttributes
                    ->firstWhere('pivot.attribute_id', 13)?->pivot->value;

                return $size ?: '&mdash;';
            })
            ->editColumn('tire_year', function (Product $item) {
                return optional(
                    $item->specificationAttributes->firstWhere('pivot.attribute_id', 15)
                )->pivot->value ?? '—';
            })
    ...
    public function getFilters(): array
    {
        ...
        $data['dimension_id'] = array_merge($data['dimension_id'], [
            'type' => 'select-ajax',
        ]);
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            'dimension_id' => [
                'title' => trans('plugins/ecommerce::products.dimension'),
                'type' => 'select-ajax',
                'validate' => 'required',
                'callback' => function (int|string|null $value = null): array {
                    $dimensionSelected = [];
                    if ($value && $dimension = Dimension::query()->find($value)) {
                        $dimensionSelected = [$dimension->getKey() => $dimension->name];
                    }

                    return [
                        'url' => route('dimensions.search'),
    ...
    ): EloquentRelation|EloquentBuilder|QueryBuilder
    {
        switch ($key) {
            ...
            case 'dimension':
                ...
                return $query->where('ec_products.dimension_id', $operator, $value);
