...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function getProductByDimensions(array $params)
    {
        $params = array_merge([
            'dimension_id' => null,
            'condition' => [],
        ...
        $filters = ['dimensions' => (array) $params['dimension_id']];

        Arr::forget($params, 'dimension_id');
            ...
            if (is_plugin_active('language') && is_plugin_active('language-advanced') && Language::getCurrentLocale() != Language::getDefaultLocale()) {
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial): void {
                        ...
                        if (in_array('dimension', $searchProductsBy)) {
                            $function = $hasWhere ? 'orWhereHas' : 'whereHas';
                            $hasWhere = true;

                            $query->{$function}('dimension.translations', function (EloquentBuilder $query) use ($keyword): void {
                                $query->where(function (BaseQueryBuilder $subQuery) use ($keyword): void { // @phpstan-ignore-line
                                    $subQuery->addSearch('name', $keyword, false);
                                });
                            });
                ...
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial): void {
                        ...
                        if (in_array('dimension', $searchProductsBy)) {
                            ...
                            $query->{$function}('dimension', function ($query) use ($keyword): void {
                                $query->where(function ($subQuery) use ($keyword): void {
                                    $subQuery->addSearch('name', $keyword, false);
                                });
                            });
...
//dd($this->model);
        ...
        $filters['dimensions'] = array_filter($filters['dimensions'], function ($item) {
            return is_numeric($item);
        });

        $filters['dimensions'] = array_filter($filters['dimensions']);

        if ($filters['dimensions']) {
            $this->model = $this->model
                ->whereIn('ec_products.dimension_id', $filters['dimensions']);
        }
        $filters['fitments'] = array_filter($filters['fitments']);
        if ($filters['fitments']) {
            $optionIds = $filters['fitments'];

            $productIds = DB::table('ec_product_fitment_attribute')
                ->select('product_id')
                ->whereIn('option_id', $optionIds) // فقط رکوردهایی که یکی از این option_idها را دارند
                ->groupBy('product_id')            // گروه‌بندی بر اساس product_id
                ->havingRaw('COUNT(DISTINCT option_id) = ?', [count($optionIds)]) // فقط محصولاتی که دقیقاً ۴ تا option از این لیست دارند
                ->pluck('product_id');             // لیست product_idها را استخراج می‌کند

//            dd($productIds);
            $this->model = $this->model
                ->whereIn('ec_products.id', $productIds);
        }
//        dd($filters,array_filter($filters['fitments']));
