...
class ProductForm extends FormAbstract
{
    public function setup(): void
    {
        ...
        $dimensions = Dimension::query()->pluck('name', 'id')->all();
        ...
        $this
            ...
            ->add(
                ...
                OnOffField::class,
                OnOffFieldOption::make()
                    ->label(trans('core/base::forms.has_qouta'))
                    ->defaultValue(false)
            )
            ->add(
                'ntsw_id',
                ...
                TextareaFieldOption::make()
                    ->label(trans('core/base::forms.ntsw_id'))
            )
            ->add(
                'product_code',
                TextField::class,
                TextareaFieldOption::make()
                    ->label(trans('core/base::forms.product_code'))
                    ->defaultValue(false)
            )
            ->add(
                'batch_name',
                TextField::class,
                TextareaFieldOption::make()
                    ->label(trans('core/base::forms.batch_name'))
                    ->defaultValue(false)
            )
            ->add(
                'batch_code',
                TextField::class,
                TextareaFieldOption::make()
                    ->label(trans('core/base::forms.batch_code'))
                    ->defaultValue(false)
            )
            ->add(
                'categories[]',
            ...
            ->when($dimensions, function () use ($dimensions): void {
                $this
                    ->add(
                        'dimension_id',
                        SelectField::class,
                        SelectFieldOption::make()
                            ->label(trans('plugins/ecommerce::products.form.dimension'))
                            ->choices($dimensions)
                            ->searchable()
                            ->emptyValue(trans('plugins/ecommerce::dimensions.select_dimension'))
                            ->allowClear()
                    );
            })
            ->add(
        ...
        if (EcommerceHelper::isProductFitmentEnabled()) {
            $this->addMetaBox(
                MetaBox::make('product-fitment-table')
                    ->title(trans('plugins/ecommerce::product-fitment.fitment_tables.title'))
                    ->hasTable()
                    ->attributes(['class' => 'product-fitment-table'])
                    ->headerActionContent(view('plugins/ecommerce::products.partials.fitment-table.header', [
                        'model' => $this->getModel(),
                        'tables' => FitmentTable::query()->pluck('name', 'id'),
                    ])->render())
                    ->content(view('plugins/ecommerce::products.partials.fitment-table.content', [
                        'model' => $this->getModel(),
                        'getTableUrl' => route('ecommerce.fitment-tables.index'),
                    ])->render())
            );
        }
        ...
        if (! $totalProductVariations) {
            ...
            ->addAfter('dimension_id', 'sku', TextField::class, TextFieldOption::make()->label(trans('plugins/ecommerce::products.sku')));
        }
