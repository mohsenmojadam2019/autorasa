...
//                }
//            }else{
//                if ($product->maximum_order_quantity > 0 && $quantityOfProduct > remain_qouta(\auth()->guard('customer')->id())) {
//                    $message = __('You cannot buy more than :quantity.', [
//                        'quantity' =>$product->has_qouta>0? remain_qouta(\auth()->guard('customer')->id()):($product->maximum_order_quantity>0?$product->maximum_order_quantity:1000),
//                    ]);
//                }
//            }
// ###
            if (!auth()->guard('customer')->check()) {
                if ($product->maximum_order_quantity > 0 && $quantityOfProduct > $product->maximum_order_quantity) {
                    $message = __('شما نمی‌توانید بیش از :quantity عدد از این محصول خریداری کنید.', [
                        'quantity' => $product->maximum_order_quantity,
                    ]);
                }
            } else {
                if ($product->maximum_order_quantity > 0 && $quantityOfProduct > remain_qouta(auth()->guard('customer')->id())) {
                    $message = __('شما نمی‌توانید بیش از :quantity عدد از این محصول خریداری کنید.', [
                        'quantity' => $product->has_qouta > 0
                            ? remain_qouta(auth()->guard('customer')->id())
                            : ($product->maximum_order_quantity > 0 ? $product->maximum_order_quantity : 1000),
                    ]);
                }
            }
  ...
  if (Auth::user()->hasPermission('dimensions.index')) {
            Menu::registerMenuOptions(Dimension::class, trans('plugins/ecommerce::dimensions.menu'));
        }
