...
class EcommerceHelper
{
    ...
    {
        if ($request->filled('fitments') && is_string($request->input('fitments'))) {
            $request->merge([
                'fitments' => json_decode($request->input('fitments'), true),
            ]);
        }
...
//        dd($request,$validator->fails());
    ...
    public function isEnabledFilterProductsByDimensions(): bool
    {
        return (bool) get_ecommerce_setting('enable_filter_products_by_dimensions', true);
    }
...
//        dd($c);
    ...
    public function dimensionsForFilter(array $categoryIds = []): Collection
    {
//        dd($this->isEnabledFilterProductsByDimensions());
        if (! $this->isEnabledFilterProductsByDimensions()) {
            return collect();
        }
...
//        dd($c);
    ...
    public function dataForFilter(?ProductCategory $category): array
    {
        ...
        $dimensions = $this->dimensionsForFilter($categoryIds);
    ...
    public function isProductFitmentEnabled(): bool
    {
        return (bool) get_ecommerce_setting('enable_product_fitment', false);
    }
}
