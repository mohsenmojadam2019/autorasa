    ...
    public function handle(Slug|array $slug): array|Slug
    {
        ...
        switch ($slug->reference_type) {
            ...
            case Dimension::class:
                ...
                $request->merge(['dimensions' => array_merge((array)$request->input('dimensions', []), [$dimension->getKey()])]);
                $products = app(GetProductService::class)->getProduct(
                    $request,
                    null,
                    $dimension->getKey(),
                ...
                SeoHelper::setTitle($dimension->name)->setDescription($dimension->description);

                Theme::breadcrumb()->add($dimension->name);

                $meta = new SeoOpenGraph();
                if ($dimension->logo) {
                    $meta->setImage(RvMedia::getImageUrl($dimension->logo));
                }
                $meta->setDescription($dimension->description);
                $meta->setUrl($dimension->url);
                $meta->setTitle($dimension->name);

                SeoHelper::setSeoOpenGraph($meta);

                SeoHelper::meta()->setUrl($dimension->url);

                if (function_exists('admin_bar')) {
                    admin_bar()
                        ->registerLink(
                            trans('plugins/ecommerce::dimensions.edit_this_dimension'),
                            route('dimensions.edit', $dimension->getKey()),
                            null,
                            'dimensions.edit'
                        );
                }

                do_action(BASE_ACTION_PUBLIC_RENDER_SINGLE, BRAND_MODULE_SCREEN_NAME, $dimension);

                return [
                    'view' => 'ecommerce.dimension',
                    'default_view' => 'plugins/ecommerce::themes.dimension',
                    'data' => compact('dimension', 'products'),
                    'slug' => $dimension->slug,
                ];
