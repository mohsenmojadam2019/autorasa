...
{{--                            <strong>{{ $order->created_at->translatedFormat('d M Y H:i:s') }}</strong>--}}
                    ...
                    @if ($order->address->name)
                        <div class="col-md-6">
                            ...
                            @if ($order->address && $order->address->date && $order->address->time)
                                <div class=" mt-3">
                                    {{ __('تاریخ ارسال:') }} {{ \Carbon\Carbon::parse($order->address->date)->format('Y/m/d') }}<br>
                                    {{ __('زمان ارسال:') }} {{ $order->address->time }}
                                </div>
                            @endif
                        </div>
                    @endif
