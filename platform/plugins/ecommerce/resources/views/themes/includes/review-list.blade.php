...
{{--                <time class="text-muted small" datetime="{{ $review->created_at->translatedFormat('Y-m-d\TH:i:sP') }}">--}}
                <time class="text-muted small" datetime="{{ $review->created_at }}">
                    {{ $review->created_at }}
                </time>
                @if ($review->order_created_at)
                    <div class="small text-muted">{{ __('✅ Purchased :time', ['time' => $review->order_created_at]) }}</div>
                @endif
...
{{--                            <time class="text-muted small" datetime="{{ $review->reply->created_at->translatedFormat('Y-m-d\TH:i:sP') }}">--}}
                            <time class="text-muted small" datetime="{{ $review->reply->created_at }}">
