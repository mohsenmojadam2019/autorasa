...
use Botble\Kyc\Forms\KYCSettingForm;
use Botble\Kyc\Forms\MediaSettingForm;
use Botble\Kyc\Http\Requests\KYCSettingRequest;
use Botble\Kyc\Http\Requests\MediaSettingRequest;
...
class KYCSettingController extends SettingController
{
    ...
    {
        $this->pageTitle(trans('plugins/kyc::kyc.media.title'));
//dd('edit');
        $form =  KYCSettingForm::create();

        return view('core/setting::media', compact('form'));
    }

    public function update(KYCSettingRequest $request): BaseHttpResponse
    {
//        dd($request);
        $this->saveSettings([
            ...$request->validated(),
