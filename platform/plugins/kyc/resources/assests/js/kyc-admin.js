...
$(() => {
    document.addEventListener('core-table-init-completed', function (event) {
        new KycAdminManagement().init(event.detail.table.prop('id'))
    })

    $(document)
        .on('show.bs.modal', '#kyc-field-modal', (e) => {
        ...
        .on('click', '#kyc-field-modal button[type="submit"]', (e) => {
            e.preventDefault()
            ...
            $httpClient
                ...
                .then(({ data }) => {
                    ...
                    $('#botble-kyc-tables-kyc-field-table').DataTable().draw()
                })
        })
})
...
$(() => {
    ...
    $(document)
        .on('show.bs.modal', '#kyc-group-field-modal', (e) => {
            console.log('tests');
            const modal = $(e.currentTarget)
            const href = $(e.relatedTarget).prop('href')
            $httpClient
        ...
        .on('click', '#kyc-group-field-modal button[type="submit"]', (e) => {
            e.preventDefault()
            ...
            $httpClient
                ...
                .then(({ data }) => {
                    ...
                    $('#botble-kyc-tables-kyc-group-field-table').DataTable().draw()
                })
        })
})
