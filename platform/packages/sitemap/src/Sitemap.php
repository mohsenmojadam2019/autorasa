...
//refactor
    ...
    public function render(string $format = 'xml'): Response
    {
        ...
        $property = $reflection->getProperty('sitemaps');
        $property->setAccessible(true);

        $filteredSitemaps = array_filter($property->getValue($this->model), function ($item) {
            return preg_match('/^\d{4}-\d{2}-\d{2}/', $item['lastmod']);
        });
        $property->setValue($this->model, $filteredSitemaps);
        $data = $this->generate($format);
        return $this->response->make($data['content'], 200, $data['headers']);
    }
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//
//        $filteredSitemaps[] = [
//            'loc' => 'http://localhost:8210/sitemap_index.xml', // URL
//            'lastmod' => now()->toDateTimeString() // تاریخ فعلی
//        ];
//
//        $property->setValue($this->model, $filteredSitemaps);
//
...
/*        $xmlContent .= '<?xml-stylesheet href="' . url('vendor/core/packages/sitemap/styles/sitemapindex.xsl') . '" type="text/xsl"?>' . "\n";*/
...
//            $xmlContent .= '        <lastmod>' . htmlspecialchars($sitemap['lastmod']) . '</lastmod>' . "\n";
//            $xmlContent .= '    </sitemap>' . "\n";
//        }
//
