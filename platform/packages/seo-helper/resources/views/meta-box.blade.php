...
>
    ...
    <div @class(['existed-seo-meta', 'hidden' => empty($object->id)])>
        @if ($meta['index'] === 'noindex')
            <span class="page-index-status me-3">
                <x-core::icon name="ti ti-search-off" class="text-warning" size="sm"/>
                {{ trans('packages/seo-helper::seo-helper.noindex') }}
            </span>
        @else
            <span class="page-index-status me-3">
                <x-core::icon name="ti ti-search" class="text-success" size="sm"/>
                {{ trans('packages/seo-helper::seo-helper.index') }}
            </span>
        @endif

        @if (($meta['follow'] ?? 'follow') === 'nofollow')
            <span class="page-follow-status">
                <x-core::icon name="ti ti-link-off" class="text-danger" size="sm"/>
                {{ __('دنبال نکردن لینک (nofollow)') }}
            </span>
        @else
            <span class="page-follow-status">
                <x-core::icon name="ti ti-link" class="text-success" size="sm"/>
                {{ __('دنبال کردن لینک (follow)') }}
            </span>
        @endif

        <h4 class="page-title-seo text-truncate mt-3">
        ...
        <div>
            <span style="color: #70757a;">
                {{ !empty($object->id) && $object->created_at ? $object->created_at : \Carbon\Carbon::now() }} -
            </span>
